...
class MoisesDB:
    def __init__(self, data_path=default_data_path, sample_rate=default_sample_rate):
        ...
        self.provider_tracks = self.get_tracks_list(self.providers_list)
        self.tracks = self.build_tracks(self.provider_tracks)
    ...
    def get_provider_tracks(self, provider):
        fs = get_fs(self.data_path)
        provider_tracks = [
    ...
    def get_tracks_list(self, providers_list):
        return {p: self.get_provider_tracks(provider=p) for p in providers_list}

    def build_tracks(self, provider_tracks):
        tracks = {}
        for provider, tracks_list in provider_tracks.items():
            tracks[provider] = [
                MoisesDBTrack(
                    provider=provider,
                    track_id=t,
